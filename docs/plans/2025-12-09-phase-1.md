...
**Character Creation:**
- Create character of each class
- Verify attributes, equipment, AC, HP calculated correctly
- Test Fighter with each feat option

**Combat Mechanics:**
- Test initiative system with different DEX scores, Perception bonus, ties
- Test critical hits (verify double damage dice)
- Test all 4 critical fumble effects
- Test saving throws (Fort/Reflex/Will) against various DCs

**Class Abilities:**
- Fighter: Second Wind heals correct amount, Power Attack math
- Rogue: Sneak Attack applies when conditions met, Dodge works, Evasion
- Wizard: Spell slots deplete and restore correctly
- Cleric: Channel Energy, Turn Undead vs Skeleton

**Conditions:**
- Apply each condition
- Verify effects (damage, bonuses/penalties)
- Test duration countdown
- Test stacking rules

**Rest System:**
- Short rest restores encounter abilities only
- Long rest restores everything

### Step 2 Tests

**Spellcasting:**
- Cast each spell, verify damage/effects/conditions
- Test spell save DCs (enemies resist/fail appropriately)
- Test Arcane Recovery (spell slot restored on kill)
- Test spell slot depletion (can't cast when out)
- Test cantrips (unlimited usage)

### Step 3 Tests

**Enemy Variety:**
- Fight each enemy type, verify tactics work
- Test Turn Undead vs Skeleton/Wraith
- Test Poison, Silence, Weakened conditions in combat
- Test enemy spellcasting (Dark Cultist uses spells intelligently)
- Test immunities (Skeleton immune to Poison/Sleep)
