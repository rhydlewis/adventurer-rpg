...
1. **Permadeath handling:** When player dies in combat, do we show a narrative "death scene" or go straight to game over?

2. **Save/Load:** Should world state be persisted to localStorage during Phase 2, or wait for a dedicated save system?

3. **Combat return point:** When combat ends, should we return to the same node or advance to a post-combat node?
